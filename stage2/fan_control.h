...
static void fan_control(uint64_t arg0)
{
	...
	while(fan_control_running)
	{
		...
		if(fan_control_running) // Avoids loading previous mode (By Evilnat)
		{
			...
			else if(t_cpu <= max_temp)
			{
				if(t_cpu >= (max_temp - 2)) continue;
				warning = 0;
				speed = speed - 4; // -1.5%
			...
			{
				// Overheating
				// Let's warn the user with triple beep and we set fan speed to 0xF6
...
static void start_control(uint64_t arg0)
{
	...
	if(!fan_control_running)
		ppu_thread_create(&fan_control_id, fan_control, (uint64_t)arg0, -0x1D8, 0x4000, 1, "fan_control");
...
void do_fan_control(int mode)
{
	...
	if(!fan_control_running)
		ppu_thread_create(&control_id, start_control, (uint64_t)mode, -0x1D8, 0x4000, 1, "start_control");
}
...
void load_fan_control(int mode)
{
	if(set_fan_speed == 0) // Disabled
		return;
	else if(set_fan_speed >= 0x33 && set_fan_speed <= 0xFE) // Manual mode
		sm_set_fan_policy(0, 2, set_fan_speed);
	else if(set_fan_speed == 1)  // SYSCON mode
